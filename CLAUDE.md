...
- browse: `↑↓`/`jk` 行移動（メッセージ境界を自動で越える。`e`/`y`/`h`/`l` はカーソル行が属するメッセージが対象）、`←→`/`hl` 兄弟ブランチ切り替え、`e` 分岐編集、`Ctrl+E`/`Ctrl+Y` 1 行スクロール下/上

**ブラウズカーソル**: `(メッセージ index, メッセージ内の論理行)` の 2 次元で、`ChatView._cursor_msg` / `_cursor_line` が保持する。折り返し表示は 1 論理行として扱う（視覚行単位ではない）。選択表示は行単位の背景ハイライト（`_highlight_line`）で、行数は `update()` 時に表示テキストから算出（ツール呼び出し表示行を含む）。スクロール追従は各メッセージの `FormattedTextControl.get_cursor_position` がカーソル行を返すことで `ScrollablePane` が行単位に行う。browse 進入時のカーソル初期位置は末尾メッセージの最終行。

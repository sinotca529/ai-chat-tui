...
- browse: `↑↓`/`jk` 行移動（メッセージ境界を自動で越える。`e`/`y`/`h`/`l` はカーソル行が属するメッセージが対象）、`{`/`}`（エイリアス `[[`/`]]`。JIS 配列では後者が Shift 不要）前/次のメッセージ先頭へ（`{` は vim 同様、メッセージ途中ではまず現在メッセージの先頭へ）、`gg`/`G` 先頭/末尾の視覚行へ移動、`←→`/`hl` 兄弟ブランチ切り替え、`e` 分岐編集、`Ctrl+E`/`Ctrl+Y` 1 行スクロール下/上（vim 同様、カーソルがビュー外に出る場合は `scroll_offsets` の内側まで引きずられる。カーソルが見えている限り ScrollablePane は手動スクロールを上書きしない）、`Ctrl+D`/`Ctrl+U` 半ページ下/上（スクロールとカーソルを同量動かし画面内相対位置を保つ。端ではスクロールのみクランプされカーソルは端まで進む。`Ctrl+D` の送信は input モード専用なので衝突しない）

**ブラウズカーソル**: `(メッセージ index, 論理行, 折り返しセグメント)` の 3 次元（`ChatView._cursor_msg` / `_cursor_line` / `_cursor_seg`）で、移動は**視覚行単位**（vim の gj/gk 相当）。折り返し位置は `_wrap_starts` が計算する。これは prompt_toolkit の Window と**同一の幅関数**（`prompt_toolkit.utils.get_cwidth`、全角 = 2 セル）と**同一の貪欲法**の写しで、幅は直近描画の `render_info.window_width`（未描画時は 80）、継続行は行プレフィックス分（2 セル）狭い。**写しゆえに pt 側の変更でサイレントにずれるリスクがあるため、実描画行数と照合するオラクルテスト（`test_wrap_math_matches_renderer`）で整合を CI 検証する**。選択表示はカーソルのある視覚行の文字範囲のみ背景ハイライト。**メッセージ末尾の空行にもカーソルが乗る**（移動は常に 1 視覚行ずつでスキップしない。空行では 1 セルのスペースをハイライトして位置を示す。空行はそのメッセージに属し `e`/`y`/`h`/`l` の対象になる）。スクロール追従は `get_cursor_position` が (セグメント開始文字, 論理行) を返すことで行われる。browse 進入時の初期位置は末尾メッセージの最終テキスト行（`G` も同じ。空行は移動でのみ到達する）。**最終視覚行（末尾メッセージの空行）で `j` は停止する**（「選択なし」の番兵 -1 には遷移しない。番兵化するとフォーカス中 Window のカーソル位置が pt のデフォルト (0,0) に落ち、keep_cursor_visible がそのメッセージ先頭までビューを巻き戻すため。番兵はブラウズ未進入の初期状態のみ）。`h`/`l` のブランチ切替後は Window が作り直されるため、必ずカーソル位置の Window へフォーカスを移し直す。
